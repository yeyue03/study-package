<template>
  ...
  >
    <template #footer>
      <a-checkbox v-model:checked="recreate" v-if="addType === 'add'"
        >保存后再创建一个</a-checkbox
      >
    ...
    <a-form ref="formRef" :label-col="labelCol" :wrapper-col="wrapperCol">
      <a-form-item label="状态" v-bind="validateInfos.status" name="status">
        <template v-if="addType == 'detail'">{{
          formState.status == 1 ? "启用" : "停用"
        }}</template>
        <a-select
          v-else
      ...
      <a-form-item
        label="方案名称"
        v-bind="validateInfos.zoneSchemeName"
        name="zoneSchemeName"
      >
        <template v-if="addType == 'detail'">{{
          formState.zoneSchemeName
        }}</template>
        <a-input
          v-else
          v-model:value="formState.zoneSchemeName"
          :maxLength="50"
        ></a-input>
      </a-form-item>
